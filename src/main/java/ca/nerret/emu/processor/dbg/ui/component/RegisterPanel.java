...
class RegisterPanel extends JPanel {
    ...
    private void drawRegisters(Graphics g, Point point) {
        ...
        //drawByte(g, new Point(secondByteColumn, yLocation), ralu.getRegister(Registers.Register.ACCUMULATOR).getRawValue(), Registers.Register.ACCUMULATOR.getDescription());

        yLocation += rowSize;
        //drawByte(g, new Point(secondByteColumn, yLocation), ralu.getRegister(Registers.Register.X_INDEX).getRawValue(), Registers.Register.X_INDEX.getDescription());

        yLocation += rowSize;
       // drawByte(g, new Point(secondByteColumn, yLocation), ralu.getRegister(Registers.Register.Y_INDEX).getRawValue(), Registers.Register.Y_INDEX.getDescription());

        yLocation += rowSize;
        drawByte(g, new Point(xLocation, yLocation), 0b00000001, "");
       // drawByte(g, new Point(secondByteColumn, yLocation), ralu.getRegister(Registers.Register.STACK_POINTER_LOW).getRawValue(), Registers.Register.STACK_POINTER_LOW.getDescription());

        //TODO this needs a combined value display
        yLocation += rowSize;
        //drawByte(g, new Point(xLocation, yLocation), ralu.getProgramCounter(), Registers.Register.PROGRAM_COUNTER_HI.getDescription());
        //drawByte(g, new Point(secondByteColumn, yLocation), ralu.getRegister(Registers.Register.PROGRAM_COUNTER_LOW).getRawValue(), Registers.Register.PROGRAM_COUNTER_LOW.getDescription());
        drawByte(g, new Point(xLocation, yLocation), ralu.getProgramCounter().getRawValue(), "Program Counter");
        ...
        //drawFlags(g, new Point(secondByteColumn, yLocation), ralu.getAlugetRegister(Registers.Register.STATUS_FLAGS).getRawValue(), "NV BDIZC".toCharArray());
        drawFlags(g, new Point(secondByteColumn, yLocation), ralu.getPSW().getRawValue(), "NV BDIZC".toCharArray());
    }
    ...
    public void setRegisters(EEC8061_RALU ralu) {
        this.ralu = ralu;
    }
